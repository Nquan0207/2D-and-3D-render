...
class UManager(object):
    ...
    * second call to setup_texture: activate GL.GL_TEXTURE2
    ...
    def setup_texture(self, sampler_name, image_file):
        ...
        # Ensure correct row alignment for RGB images (3 bytes per pixel)
        GL.glPixelStorei(GL.GL_UNPACK_ALIGNMENT, 1)

        GL.glTexImage2D(
            GL.GL_TEXTURE_2D, 0, GL.GL_RGB, rgb_image.shape[1],
            rgb_image.shape[0], 0, GL.GL_RGB, GL.GL_UNSIGNED_BYTE, rgb_image,
        )
        # Filtering
        GL.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MIN_FILTER, GL.GL_LINEAR)
        GL.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MAG_FILTER, GL.GL_LINEAR)
        # Clamp edges to avoid sampling across opposite edges at u/v = 0 or 1
