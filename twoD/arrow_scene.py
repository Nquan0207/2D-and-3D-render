...
class ArrowScene(BaseScene3D):
    def __init__(self, renderer: Renderer) -> None:
        super().__init__(renderer, axes_scale=1.2)

        # A simple right-pointing arrow outline (concave), CCW ring
        ring = np.array(
            [
                [-0.8, -0.25, 0.0],  # tail bottom
                [-0.1, -0.25, 0.0],  # shaft end bottom
                [-0.1, -0.5, 0.0],   # lower notch
                [0.85, 0.0, 0.0],    # tip
                [-0.1, 0.5, 0.0],    # upper notch
                [-0.1, 0.25, 0.0],   # shaft end top
                [-0.8, 0.25, 0.0],   # tail top
            ],
            dtype=np.float32,
        )
        # Triangulate concave arrow by fan from center (as GL_TRIANGLES)
