...
def _fan_vertices(n: int, radius: float = 0.8, start_angle: float = 0.0) -> np.ndarray:
    angles = start_angle + np.linspace(0.0, 2.0 * np.pi, n, endpoint=False)
    ring = np.stack([radius * np.cos(angles), radius * np.sin(angles), np.zeros_like(angles)], axis=1)
    center = np.array([[0.0, 0.0, 0.0]], dtype=np.float32)
    # Close the fan loop: repeat the first ring vertex
    return np.vstack([center, ring, ring[:1]]).astype(np.float32)


class PentagonScene(BaseScene3D):
    def __init__(self, renderer: Renderer) -> None:
        super().__init__(renderer, axes_scale=1.2)

        positions = _fan_vertices(5, radius=0.8, start_angle=np.pi / 2.0)
